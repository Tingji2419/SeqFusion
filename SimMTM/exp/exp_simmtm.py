from SimMTM.data_provider.data_factory import data_provider
from SimMTM.exp.exp_basic import Exp_Basic
from SimMTM.utils.tools import EarlyStopping, adjust_learning_rate, transfer_weights, show_series, show_matrix
from SimMTM.utils.augmentations import masked_data
from SimMTM.utils.metrics import metric
...
class Exp_SimMTM(Exp_Basic):
    ...
    def pretrain(self, train_data):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            end_time = time.time()
            print(
                "Epoch: {0}, Lr: {1:.7f}, Time: {2:.2f}s | Train Loss: {3:.4f}/{4:.4f}/{5:.4f}".format(epoch, model_scheduler.get_lr()[0], end_time - start_time, train_loss, train_cl_loss,
                        train_rb_loss))
            # print(
    ...
    def pretrain_one_epoch(self, train_loader, model_optim, model_scheduler):
        ...
        for i, batch_x in enumerate(train_loader):
            ...
            # encoder
            loss, loss_cl, loss_rb, _, _, _, _ = self.model(batch_x_om, None, batch_x, mask=mask_om)
    ...
    def valid_one_epoch(self, vali_loader):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            # data augumentation
            batch_x_m, mask = masked_data(batch_x, self.args.mask_rate, self.args.lm,
    ...
    def show(self, num, epoch, type='valid'):
        ...
        # show cases
        if type == 'valid':
            batch_x = self.valid_show
        else:
            batch_x = self.train_show
        ...
        # Encoder
        with torch.no_grad():
            loss, loss_cl, loss_rb, positives_mask, logits, rebuild_weight_matrix, pred_batch_x = self.model(batch_x_om, None, batch_x, mask=mask_om)
        ...
        fig_logits.savefig(f'{epoch}_fig_logits.png')
        fig_positive_matrix.savefig(f'{epoch}_fig_positive_matrix.png')
        fig_rebuild_weight_matrix.savefig(f'{epoch}_fig_rebuild_weight_matrix.png')
