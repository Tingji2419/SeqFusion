...
class ContrastiveZooWeight(nn.Module):

    def __init__(self, args):
        super(ContrastiveZooWeight, self).__init__()
        self.temperature = args.temperature
        self.softmax = torch.nn.Softmax(dim=-1)
        self.log_softmax = torch.nn.LogSoftmax(dim=-1)
        self.kl = torch.nn.KLDivLoss(reduction='batchmean')
    ...
    def forward(self, batch_emb_om, batch_dual_mse):
        
        batch_emb = batch_emb_om[:(batch_emb_om.shape[0] // (self.positive_nums + 1)), ]
        # get similarity matrix among mask samples
        norm_emb = F.normalize(batch_emb, dim=1)
        similarity_matrix = torch.matmul(norm_emb, norm_emb.transpose(0, 1))

        # generate predict and target probability distributions matrix
        y_true = 1 - batch_dual_mse
        # Create a mask to zero out diagonal elements
        mask = torch.eye(similarity_matrix.size(0), dtype=torch.bool, device=similarity_matrix.device).logical_not()
        masked_similarity_matrix = similarity_matrix[mask].view(similarity_matrix.size(0), -1).float()
        masked_y_true = y_true[mask].view(y_true.size(0), -1).float()
        loss = F.mse_loss(masked_similarity_matrix, masked_y_true).float()
        # # multiple positives - KL divergence
        # predict = self.log_softmax(masked_logits / self.temperature)
        # loss = self.kl(predict, masked_y_true)
