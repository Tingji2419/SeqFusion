...
from SimMTM.layers.Transformer_EncDec import Decoder, DecoderLayer, Encoder, EncoderLayer
from SimMTM.layers.SelfAttention_Family import DSAttention, AttentionLayer
from SimMTM.layers.Embed import DataEmbedding
from SimMTM.utils.losses import AutomaticWeightedLoss
from SimMTM.utils.tools import ContrastiveWeight, AggregationRebuild, ContrastiveZooWeight
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'pretrain':
            ...
            self.contrastive_zoo = ContrastiveZooWeight(self.configs)
    ...
    def pretrain(self, x_enc, x_mark_enc, batch_x, mask, batch_dual_mse=None):
        ...
        loss_zoo = self.contrastive_zoo(s_enc_out, batch_dual_mse)
        ...
        # loss
        loss = self.awl(loss_cl, loss_rb, loss_zoo)

        return loss, loss_cl, loss_rb, positives_mask, logits, rebuild_weight_matrix, pred_batch_x

    def forward(self, x_enc, x_mark_enc, batch_x=None, mask=None, batch_dual_mse=None):

        if self.task_name == 'pretrain':
            return self.pretrain(x_enc, x_mark_enc, batch_x, mask, batch_dual_mse)
    ...
    def extract_feature(self, x_enc):
        # data shape
        bs, seq_len, n_vars = x_enc.shape
