...
from transformer_models.models import FEDformer, Autoformer, Informer, Transformer, DLinear, Mean, SeasonalNaive, Last
...
class Exp_Transfer(Exp_Basic):
    def __init__(self, args):
        super(Exp_Transfer, self).__init__(args)
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                # print(batch_x[0, :, -1], batch_y[0, :, -1], outputs[0, :, -1])
                # break
        ...
        if test:
            ...
            if self.args.source_data is not None:
                ...
                if args.target_data != args.data:
                    args.seasonal_patterns = args.source_freq
                model_path = setting_string(args, 0)
            else:
                model_path = setting
            if self.args.use_gpu:
                self.model.load_state_dict(torch.load(os.path.join(
                    './checkpoints/' + model_path, 'checkpoint.pth')))
            else:
                self.model.load_state_dict(torch.load(os.path.join(
                    './checkpoints/' + model_path, 'checkpoint.pth'), map_location=torch.device('cpu')))
        preds = []
        trues = []
